...
function setCachePath {
    declare envOS
    envOS=$(uname -s)
    declare pkgDir="aries-framework-go-ext"
...
function recordChannelCacheResult {
    declare FIXTURES_TREE_CHANNEL
    FIXTURES_TREE_CHANNEL=$(ls -R test/bdd/fixtures/fabric/channel)
...
function recordCryptoCacheResult {
    declare FIXTURES_TREE_CRYPTO
    FIXTURES_TREE_CRYPTO=$(ls -R test/bdd/fixtures/fabric/crypto-config)
...
function isCryptoFixturesCurrent {
    if [ ! -d "test/bdd/fixtures/fabric/crypto-config" ]; then
    ...
    declare FIXTURES_TREE
    FIXTURES_TREE=$(ls -R test/bdd/fixtures/fabric/crypto-config)
    declare cachedFixturesTree
    cachedFixturesTree=$(< "${CACHE_PATH}/${FIXTURES_CRYPTO_TREE_FILENAME}")
...
function isChannelFixturesCurrent {
    if [ ! -d "test/bdd/fixtures/fabric/channel" ]; then
    ...
    declare FIXTURES_TREE
    FIXTURES_TREE=$(ls -R test/bdd/fixtures/fabric/channel)
    declare cachedFixturesTree
    cachedFixturesTree=$(< "${CACHE_PATH}/${FIXTURES_CHANNEL_TREE_FILENAME}")
    if [ "${FIXTURES_TREE}" != "${cachedFixturesTree}" ]; then
